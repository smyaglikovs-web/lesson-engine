...
const EditableBlockCard = ({ block, onChange }) => {
  ...
  // VIDEO BLOCK WITH TRANSCRIPT/SUBTITLES FOR AI
  if (block.type === 'video') {
    return (
      <div className="space-y-3">
        ...
        <div>
          <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1">
            📝 Субтитры / Транскрипт / Содержание видео (для AI Помощника)
          </label>
          <textarea
            rows="3"
            value={block.transcript || ''}
            onChange={e => onChange({ ...block, transcript: e.target.value })}
            placeholder="Вставьте субтитры или описание видео сюда, чтобы AI создал точные упражнения на аудирование..."
