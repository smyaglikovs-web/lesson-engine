...
export const BuilderPalette = ({ onAddBlock }) => (
  <div className="lg:col-span-1 bg-white p-5 rounded-3xl border border-slate-200 shadow-sm space-y-4 h-fit sticky top-20">
    ...
    <div className="space-y-1.5 text-xs font-semibold">
      ...
      <button onClick={() => onAddBlock('teacher_notes')} className="w-full text-left p-2.5 bg-amber-50 hover:bg-amber-100 border border-amber-200 text-amber-900 rounded-xl transition flex items-center gap-2 cursor-pointer font-bold shadow-2xs">👨‍🏫 Заметки Учителя</button>
      ...
      <button onClick={() => onAddBlock('inline_select')} className="w-full text-left p-2.5 bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 text-indigo-900 rounded-xl transition flex items-center gap-2 cursor-pointer font-bold shadow-2xs">🔽 Вставка из списка (Inline)</button>
      <button onClick={() => onAddBlock('spinning_wheel')} className="w-full text-left p-2.5 bg-gradient-to-r from-indigo-50 to-purple-50 hover:opacity-90 border border-purple-200 text-purple-900 rounded-xl transition flex items-center gap-2 cursor-pointer font-bold shadow-2xs">🎡 Колесо вопросов</button>
