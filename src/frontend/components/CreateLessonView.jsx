...
// SAFE NON-DESTRUCTIVE STRUCTURE SANITIZER
function sanitizeLessonStructure(lessonObj) {
  ...
  const sanitizedPages = pages.map((p, pIdx) => {
    const rawBlocks = Array.isArray(p.blocks) ? p.blocks : (Array.isArray(p.items) ? p.items : []);
    const sanitizedBlocks = rawBlocks.map((b, bIdx) => {
      if (!b || typeof b !== 'object') {
        return { id: `b-${pIdx}-${bIdx}`, type: 'text', text: '' };
      }
      
      const blockId = b.id || `b-${pIdx}-${bIdx}-${Date.now()}`;
      const type = normalizeBlockType(b.type);

      // PRESERVE ALL ORIGINAL BLOCK DATA! NEVER OVERWRITE BLOCK TYPE TO 'TEXT'!
      return {
        ...b,
        id: blockId,
        type: type
...
export const CreateLessonView = ({ initialLesson, onSaveLesson, onCancel }) => {
  ...
  const [currentLesson, setCurrentLesson] = useState(initialLesson ? sanitizeLessonStructure(initialLesson) : DEFAULT_NEW_JSON);
  ...
  const syncVisualBuilderChanges = (updatedLesson) => {
  ...
  const handleGenerateFullAiLesson = async () => {
    ...
    try {
      ...
      if (data.success && data.jsonText) {
        ...
        syncVisualBuilderChanges(sanitizedObj);
    ...
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      {createMode === 'visual' && (
        <VisualBuilderView
          ...
          onChangeLesson={syncVisualBuilderChanges}
          onCancel={onCancel}
        />
      )}
