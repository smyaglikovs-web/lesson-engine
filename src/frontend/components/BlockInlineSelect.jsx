  ...
  return (
    <div className="bg-white p-4 sm:p-8 rounded-3xl border border-slate-200 shadow-xs mb-6 space-y-4 w-full min-w-0 overflow-hidden">
      ...
      <div>
        ...
        <h4 className="font-extrabold text-slate-900 text-sm sm:text-base border-l-4 border-indigo-600 pl-3 leading-snug break-words">
          {block.instruction || 'Choose the correct word in context:'}
        </h4>
      </div>

      {/* READING STORY PASSAGE WITH INLINE DROPDOWNS */}
      <div className="space-y-3 text-sm sm:text-base leading-relaxed font-serif text-slate-800">
        {parsedLines.map((lineSegments, lIdx) => (
          <div key={lIdx} className="py-0.5 leading-loose">
            {lineSegments.map((seg, sIdx) => {
              ...
              return (
                <span key={seg.gapKey} className="inline-flex items-center gap-1 mx-1 flex-wrap">
                  <select
                    disabled={submitted}
                    value={selectedVal}
                    onChange={e => handleSelectChange(seg.gapKey, e.target.value)}
                    className={`px-2.5 py-1 border-2 rounded-xl text-xs sm:text-sm font-bold outline-none transition cursor-pointer shadow-2xs font-sans ${borderStyle}`}
                  >
                  ...
                  {/* REVEAL CORRECT WORD ON INCORRECT SUBMISSION */}
                  {submitted && !isCorrect && (
                    <span className="px-2 py-0.5 bg-emerald-100 text-emerald-800 font-extrabold text-xs rounded-md border border-emerald-200 shadow-2xs font-sans">
                      ✓ {seg.correct}
                    </span>
                  )}
                </span>
      ...
      {!submitted ? (
        <div className="flex gap-2 pt-2">
          <button
            ...
            className="w-full sm:w-auto px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-extrabold rounded-2xl text-xs shadow-md transition disabled:opacity-40 cursor-pointer"
          >
            Check Answers
          </button>
        ...
        <div className={`p-4 rounded-2xl text-xs font-bold flex flex-col sm:flex-row sm:items-center justify-between gap-3 ${
          correctCount === totalGaps ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-900'
        }`}>
          <span>
